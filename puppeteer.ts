...
const authenticatePageOne = async (page: puppeteer.Page) => {
    try{
...
const authenticatePageTwo = async (page: puppeteer.Page) => {
    try{
...
const searchFunction = async(page: puppeteer.Page, searchWord: string): Promise<void> => {
    try{
...
const scrapeScrollItems = async (page: puppeteer.Page, targetLength: number): Promise<void> => {
    try{
        const tweetArray = []
        while(targetLength <= 40){
            ...
            if(tweets.length>0){
                for (let i=0; i<=(tweets.length - 1); i++) {
                    try{
                        ...
                        const tweet: string = await page.evaluate(el=> el.innerText, tweets[i]);
                        let datePosted: (string|Date) = await page.evaluate(el=> el.getAttribute("datetime"), datesPosted[i]);
                        datePosted = dayjs(datePosted).add(1, 'hour').toDate()
            ...
            fs.appendFile('tweets.json', JSON.stringify(tweetArray), (err) => {
                if (err) throw err;
...
const navigateToPage = async (page: puppeteer.Page, URL: string, search: boolean, getData: boolean): Promise<void> => {
    try{
        const tweetArray = []
